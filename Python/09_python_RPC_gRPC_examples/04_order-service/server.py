...
import time
import uuid # Modulo per generare identificatori univoci.
import sys
...
class OrderManagementServicer(order_management_pb2_grpc.OrderManagementServicer): 
    ...
    def __init__(self):
        ...
        """
        # Dizionario per memorizzare gli ordini, dove 
        # la chiave è l'ID dell'ordine e il valore è l'ordine stesso.
        self.orderDict = {}                                                          
    ...
    def searchInventory(self, query):
        matchingOrders = []  # Lista per memorizzare gli ordini che corrispondono alla ricerca
        
        # Itera attraverso tutti gli ordini nel dizionario
        for order_id, order in self.orderDict.items():
            # Per ogni ordine, itera attraverso gli items
            for itm in order.items:
                # Se l'item contiene la query, aggiungi l'ordine alla lista dei matchingOrders
                if query in itm:
                    matchingOrders.append(order)
                    # Una volta trovato un item corrispondente, esci dal loop interno
                    break

        return matchingOrders  # Ritorna la lista degli ordini che contengono l'item cercato
