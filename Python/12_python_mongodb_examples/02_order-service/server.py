...
class OrderManagementServicer(order_management_pb2_grpc.OrderManagementServicer): 
    ...
    def getOrder(self, request, context):
        # Query al database MongoDB per ottenere l'ordine
    ...
    def searchOrders(self, request, context):  
        ...
        #Query al database MongoDB per cercare gli ordini che contengono l'item specificato
        orders_db_find = self.orders_collection.find({"items" : {"$in" : [request.value]}})
        # Costruzione delle risposte utilizzando i dati recuperati dal database
    ...
    def processOrders(self, request_iterator, context):
        ...
        # Elaborazione degli ordini per la spedizione e memorizzazione nel database MongoDB
        location_dict = {}
    ...
    def cancelShipment(self, request, context):
        # Cancella la spedizione dal database MongoDB
