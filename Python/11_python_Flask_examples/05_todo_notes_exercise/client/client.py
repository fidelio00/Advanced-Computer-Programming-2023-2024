...
def add_note(note):
    ...
    #una volta presa questa stringa, la passo al metodo post che mi offre il package requests

    # Make a POST request containing the note as json payload
    resp = requests.post(resourse_location, json=note)
    # Grazie a post sappiamo che è formattato secondo json

    # Check for response error
    try:
        resp.raise_for_status() #Quando viene invocato, succede che l a chiamata solleva errore HHTP Error se lo status code è 400 quindi non è andato tutto okay
...
if __name__ == "__main__":
    ...
    # Add new notes
    for i in range (2):
        note = generate_note() #Mi crea un dizionario
