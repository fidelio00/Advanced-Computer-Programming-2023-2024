...
notes = {} #dizionario definito fuori da tutte le funzioni, nello scope globale 
...
@app.post('/note')
def add_note():
    #Per aggiungere una nota, secondo la specifica chiave,valore, dobbiamo definire il tipo di nota
    #E poi devo prendere i dati che fanno parte del payload, utilizzando l'oggetto request (disponibile in flask)

    try:
        #Dovete accordarvi o capire che codifica ha il dato che vi arriva dal client, per capire come fare il parsing e decodificarlo dal lato server
        #In questo caso, client e serve possono scambiarsi attraveso formato json e vado a prendere campo text e campo tag.
